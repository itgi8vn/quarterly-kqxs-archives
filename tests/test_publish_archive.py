...
from publish_archive import SCHEMA_VERSION, publish_local, validate_url
...
class PublisherTests(unittest.TestCase):
    ...
    def publish(self, digest=None, quarter="2026-Q2"):
        return publish_local(self.db,self.manifest,self.repo,"kqxs-2026-Q2-v1",quarter,digest or self.digest,destination_root="test-output")
    ...
    def test_checked_in_fixture_url_rejected_for_production(self):
        with self.assertRaisesRegex(ValueError,"test fixtures cannot be published"):
            validate_url("https://raw.githubusercontent.com/itgi8vn/quarterly-kqxs-archives/main/test-fixtures/archive-v1/2026-Q2/manifest.json", reject_test_fixtures=True)
    ...
    def test_missing_manifest_conflict(self):
        self.publish(); (self.repo/"test-output/2026/Q2/manifest.json").unlink()
        with self.assertRaisesRegex(ValueError,"PUBLIC_CONFLICT"): self.publish()
    def test_manifest_mismatch_conflict(self):
        self.publish(); (self.repo/"test-output/2026/Q2/manifest.json").write_text("{}")
        with self.assertRaisesRegex(ValueError,"PUBLIC_CONFLICT"): self.publish()
    def test_duplicate_different_hash(self):
        self.publish(); (self.repo/"test-output/2026/Q2/kqxs_2026_Q2.sqlite").write_bytes(b"different")
        with self.assertRaisesRegex(ValueError,"PUBLIC_CONFLICT"): self.publish()
